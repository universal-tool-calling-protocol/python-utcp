...
def _ip_is_loopback_like(host: str) -> bool:
    """Mirror of ``utcp_http._security._is_loopback_host``. See that module
    for the full rationale: True if ``host`` names the machine running the
    agent, in any spelling the resolver routes locally -- ``localhost`` /
    ``127.0.0.1`` / ``::1``, a trailing-dot absolute name, ``0.0.0.0`` /
    ``::``, any 127.0.0.0/8 address, IPv4-mapped IPv6 forms, and the
    shorthand/integer/octal/hex IPv4 spellings ``inet_aton`` accepts
